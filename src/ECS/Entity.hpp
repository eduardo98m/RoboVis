...
namespace rbvs
{
   ...
   * The generation counter is used to ensure that stale entity handles (from
   * previously destroyed entities) are not mistakenly used.
   */
  struct Entity
  {
    uint32_t id; ///< Unique identifier for the entity.
    uint32_t
        generation; ///< Generation counter to invalidate stale entity references.
...
{
  template <>
  struct hash<rbvs::Entity>
  {
    /**
     * @brief Generates a hash value for an Entity.
     *
     * The hash is computed by combining the hash of the entity's id and its
     * generation.
     *
