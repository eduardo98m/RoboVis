...
namespace rbvs
{
    ...
    Matrix MakeWorldTransform(Vector3 position, Quaternion orientation, Vector3 scale)
    {
        ...
        // Raylib MatrixMultiply(A, B) is B * A. So, T * R * S means:
        // Multiply(S, R) -> R*S. Then Multiply(R*S, T) -> T*(R*S)
        return MatrixMultiply(MatrixMultiply(mat_scale, mat_rot), mat_trans);
    }

    // Overload for uniform scale (like PointClouds)
    Matrix MakeWorldTransform(Vector3 position, Quaternion orientation, float uniform_scale)
    {
        //Matrix mat_scale = MatrixScale(uniform_scale, uniform_scale, uniform_scale);
        Matrix mat_rot = QuaternionToMatrix(orientation);
        Matrix mat_trans = MatrixTranslate(position.x, position.y, position.z);
    ...
    void EntitySelectionSystem::handle_click(EntityManager &em, Camera3D camera)
    {
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT))
        {
            ...
            for (const auto e : view_vm)
            {
                ...
                Matrix world_transform = MakeWorldTransform(vm.position, vm.orientation, vm.scale);
                BoundingBox local_bbox = GetModelBoundingBox(*vm.model);
                BoundingBox world_bbox = GetTransformedBoundingBox(local_bbox, world_transform);
                RayCollision collision = GetRayCollisionBox(mouse_ray, world_bbox);

                // Alternative, potentially more accurate for complex models:
                // RayCollision collision = GetRayCollisionModel(mouse_ray, *vm.model, world_transform);
            ...
            for (const auto e : view_pc_xyz_rgba)
            {
                ...
                Matrix world_transform_pc = MakeWorldTransform(pc.position, pc.orientation, pc.scale);
                BoundingBox local_pc_bbox = ComputePointCloudBoundingBox(*pc.cloud);
                BoundingBox world_pc_bbox = GetTransformedBoundingBox(local_pc_bbox, world_transform_pc);
            ...
            for (const auto e : view_hm)
            {
                ...
                Matrix world_transform_hm = MakeWorldTransform(hm.position, hm.orientation, hm.scale);
                RayCollision collision = GetRayCollisionMesh(mouse_ray, *hm.mesh, world_transform_hm);
            ...
            if (this->selected_entity) {
                std::cout << "Selected entity ID: " << selected_entity->id << /* some way to get ID from Entity e */ std::endl;
            } else {
                std::cout << "No entity selected." << std::endl;
            }
        }
        // If you want to deselect when clicking on empty space, and nothing was hit this frame:
        // The current logic sets selected_entity to nullopt if nothing is hit.
        // If IsMouseButtonReleased is also important, you might add logic here.
    }
