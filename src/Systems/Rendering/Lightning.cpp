...
void LightingSystem::init() {
  ...
  lit.locShadowMap = GetShaderLocation(lit.litShader, "shadowMap");
  lit.locLightVP = GetShaderLocation(lit.litShader, "lightVP");
...
void LightingSystem::update_light_values(EntityManager &em,
  ...
  for (auto e : view) {
    ...
    data.position = {light.position.x, light.position.y, light.position.z,
                     1.0f};
    Vector3 tgt = light.target;

    data.target = {tgt.x, tgt.y, tgt.z, 0.0f};
  ...
  if (lit.ssboId == 0) {
    if (!lights_data.empty()) {
      lit.ssboId = rlLoadShaderBuffer(lights_data.size() * sizeof(GpuLight),
                                      lights_data.data(), RL_DYNAMIC_DRAW);
      TraceLog(LOG_INFO, "SSBO for lights created with ID: %d", lit.ssboId);
    }
  } else {
    rlUpdateShaderBuffer(lit.ssboId, lights_data.data(),
                         lights_data.size() * sizeof(GpuLight), 0);
  }
