...
namespace rbvs
{
                                                            ...
                                                            int height)
  {
    ...
    if (target.id > 0)
    {
      ...
      // rlTextureParameters(target.depth.id, RL_TEXTURE_WRAP_R, RL_TEXTURE_WRAP_CLAMP); + #define RL_TEXTURE_WRAP_R 0x8072
  ...
  RenderTexture2D load_shadow_map_point_light(int resolution)
  {
    ...
    if (target.id > 0)
    {
      ...
      target.depth.id = rlLoadTextureCubemap(
          NULL, resolution, PIXELFORMAT_UNCOMPRESSED_R32, 1);
      target.depth.width = resolution;
      target.depth.height = resolution;
      target.depth.format = PIXELFORMAT_UNCOMPRESSED_R32;
      target.depth.mipmaps = 1;

      // Setup cubemap filtering/wrapping
      rlCubemapParameters(target.depth.id, RL_TEXTURE_MIN_FILTER, RL_TEXTURE_FILTER_NEAREST);
      rlCubemapParameters(target.depth.id, RL_TEXTURE_MAG_FILTER, RL_TEXTURE_FILTER_NEAREST);
      rlCubemapParameters(target.depth.id, RL_TEXTURE_WRAP_S, RL_TEXTURE_WRAP_CLAMP);
      rlCubemapParameters(target.depth.id, RL_TEXTURE_WRAP_T, RL_TEXTURE_WRAP_CLAMP);

      // Attach depth cubemap to framebuffer
      // NOTE: We don’t attach all 6 faces here. Instead, before rendering each face,
      //       re-attach with the specific face type (POSITIVE_X, NEGATIVE_X, etc.).
      rlFramebufferAttach(
          target.id,
          target.depth.id,
          RL_ATTACHMENT_DEPTH,
          RL_ATTACHMENT_CUBEMAP_POSITIVE_X, // placeholder face
          0);
                                        ...
                                        Shader lightning_shader)
  {
    ...
    for (auto e : view)
    {
      ...
      for (auto model_entity : model_view)
      {
        ...
        // Should we do som ething if the alpha is less than 255?
        if (!vm.visible || !vm.receive_lighting)
        {
          continue;
        }
        Vector3 axis;
