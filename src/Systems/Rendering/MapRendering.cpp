...
namespace rbvs
{
    ...
    void MapRenderingSystem::update(EntityManager &em, const Camera3D &camera)
    {
        ...
        for (auto entity : view)
        {
            ...
            Color lod_map_alpha[4] = {
                {RED.r, RED.g, RED.b, 30},
    ...
    void MapRenderingSystem::update_chunks(Map &map, const Vector3 &camera_pos)
    {
        ...
        camera_local_pos = Vector3RotateByQuaternion(camera_local_pos, map.orientation);
        ...
        for (const auto &[id, chunk] : map.chunks)
        {
            ...
            {
                map.loaded_chunks[key] = std::make_unique<Model>(LoadModel(chunk.model_path.c_str()));
            }
