...
int main()
{
  ...
  float x;
  std::cout << "This is the number of points for the rendering: "
            << cloud->points.size() << "\n";
  // std::cin >> x;

  // Create a point cloud entity
  rbvs::Entity point_cloud_entity =
      visualizer.create_point_cloud<pcl::PointXYZRGBA>(
  ...
  rbvs::Entity height_map = visualizer.create_height_map(rbvs::HeightMapParams{
  ...
  // rbvs::Entity cone_entity_2 = visualizer.create_model(rbvs::ModelParams{
  //     .position = {4.0, 0.0, -2.0},
  //     .color = {100, 0, 255, 50},
  //     .model_type = rbvs::ModelType::CONE,
  //     .radius = 1.0,
  ...
  // rbvs::Entity gizmo = visualizer.create_gizmo(rbvs::GizmoParams{
  ...
  // uint cube_id = visualizer.add_box(new_pos, QuaternionIdentity(),
  // RED, 1.0, 1.0, 1.0);
  SetTargetFPS(60);
  // Main game loop
  float t_o = GetTime();
  ...
  while (!WindowShouldClose())
  {
    ...
    new_orientation =
        QuaternionFromEuler(0.0f, GetTime() * rotationSpeed, 0.0f);
    ...
    //     Color c =  latch ? (Color){.r = 0, .g = 255, .b = 58, .a = 75} :
    //     (Color){.r = 255, .g = 0, .b = 58, .a = 75};
    ...
    //     .position = std::optional<Vector3>{new_pos},  // <-- semicolon here
    //     is invalid .scale = std::optional<Vector3>{new_scale}
    // });
